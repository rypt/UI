...
class domysql():
    ...
    def select_db(self, select, num):
        if select == 'code':
            sql = "select code from hjm.h_mobile_captcha where mobile='{}' order by updated_at desc limit 1;".format(
                num)
        elif select == 'group_permission':
            sql = "SELECT group_permission FROM live.h_live_user_group where user_id ='{}' order by updated_at desc limit 1;".format(
                num)
    ...
    def delete_clean_all(self, num):
        ...
        sql5 = "delete from live.h_live_user_group_video_permission_log where user_id={};".format(num)
        sql6 = "Delete FROM hjm.h_order  where user_id in({}) and order_member_type is not null;".format(num)
        sql7 = "Delete FROM member.h_member where user_id in({});".format(num)
        sql8 = "Delete FROM member.h_member_contract where user_id in({});".format(num)
        sql9 = "Delete FROM member.h_member_privilege_level where user_id in({});".format(num)
        sql10 = "Delete FROM member.h_member_order  where user_id in({});".format(num)
        sql11 = "Delete FROM hjm.h_order_contract where user_id in({});".format(num)
        sql12 = "Delete FROM hjm.h_order  where user_id in({}) and order_member_type is not null;".format(num)
        sql_list = [sql1, sql2, sql3, sql4, sql5, sql6]
        for i in sql_list:
            sql = i
            self.cursor.execute(sql)
